...
if (port == 1)
	...
	*/
	...
	{
	/*
		We don't currently support other than UART 1 and UART 2... but we warned, the IOMux tool appears to get the code wrong. It gets the pin assignments
		tight but then get the the routing wrong.  See the two examples above for the fix... essentially to need to work out which pin in the RX pin (its the
		first line of the code generated by the IOMux tool) and then route that to RX.  In other words, duplicate the last line the IOMux tool generates, then
		change the pin to the pin names in the first line of code.
	*/
	}
}
...
/*
	ATOSE_UART_IMX6Q::READ()
	------------------------
*/
uint32_t ATOSE_uart_imx6q::read(uint8_t *buffer, uint32_t bytes)
{
uint32_t which;
...
for (which = 0; which < bytes; which++)
	...
	*/
	while ((HW_UART_USR2(port).B.RDR) == 0)
		; // do nothing

	/*
		Get it and put it into the array
	*/
	*buffer++ = HW_UART_URXD_RD(port);
	}
...
/*
	ATOSE_UART_IMX6Q::PEEK()
	------------------------
	Return the number of characters known to be in the buffer.  At present this is either 0 or 1
	depending on the status of the RDR buffer.
*/
uint32_t ATOSE_uart_imx6q::peek(void)
{
if ((HW_UART_USR2(port).B.RDR) == 0)
	return 0;
